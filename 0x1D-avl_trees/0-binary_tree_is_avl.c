...
{
	int left, right;	if (!tree || !check_order(tree->left, tree->n, 0) ||
...
{
	size_t left, right;	if (tree == NULL)
		return (0);
	if (tree->left)
...
{
	int balance, bst, left, right;	if (!tree)
		return (0);	balance = binary_tree_balance(tree);
